...
#     G = model.G
#     targets=[a for a in model.grid.get_all_cell_contents() if a.state is State.ACTIVE])
    ...
    for a in model.grid.get_cell_list_contents(G):
        if a.state != State.ACTIVE:
            if a.state != State.REMOVED:
                ...
                diffusion+=has_infective_neighbor
    # if number_active(model) != 0:
        # diffusion=diffusion/(number_active(model))
    if model.num_nodes-number_active(model)-number_removed(model) != 0:
        diffusion = diffusion / (model.num_nodes-number_active(model)-number_removed(model))
    return diffusion

def exposed_diffusion(model):
    diffusion = 0
    G = model.G
    for a in model.grid.get_cell_list_contents(G):
        if a.state != State.INACTIVE:
    ...
    diffusion = diffusion / (model.num_nodes-number_inactive(model)-number_removed(model))
    return diffusion
...
class VirusAgent(Agent):
    ...
    def find_num_neighbors(self):
        neighbors_nodes = self.model.grid.get_neighbors(self.pos, include_center=False)
        num_neighbors = [agent for agent in self.model.grid.get_cell_list_contents(neighbors_nodes)]
        self.num_neighbors = len(num_neighbors)

    def step(self):
        if self.state is State.ACTIVE:
            self.find_num_neighbors()
