...
def network_portrayal(G):
    ...
    def node_color(agent):
        if agent.state is State.SUSCEPTIBLE:
            return 'GREEN'
        elif agent.state is State.CARRIER:
            return 'YELLOW'
...
class InfectedRemainingElement(TextElement):
    def render(self, model):
        ...
        return 'Number infected: ' + str(infected)
...
model_params = {
    'num_nodes': UserSettableParameter('slider', 'Number of agents', 70, 10, 1000, 1,
    ...
    'beta': UserSettableParameter('slider', 'Virus Spread Chance', 0.013, 0.0, 0.2, 0.001,
                                                 description='Probability that susceptible neighbor will be infected'),
    'alpha': UserSettableParameter('slider', 'Recovery Chance', 0.01, 0.0, 0.2, 0.001,
                                             description='Probability that the virus will be removed'),
    'gamma': UserSettableParameter('slider', 'Gamma', 0.3, 0.0, 1.0, 0.01,
                                             description='Probability of infected becoming infective'),
}

server = ModularServer(VirusModel, [chart, *text], 'SEIR Model', model_params)
server.port = 8521