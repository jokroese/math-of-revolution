...
for j in range(100):		#number of instantiations
    # Run the model
    model = MoneyModel(50) #number of agents
...
# log_wealth= [log(element+1,10) for element in all_wealth]
...
plt.hist(all_wealth, bins=range(max(all_wealth)),
	normed=True, rwidth=1, align='left')
plt.yscale('log')
